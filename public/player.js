class WinampPlayer {
  ...
  async loadLyrics(track) {
    try {
      // Remove .mp3 extension from title for lyrics file lookup
      const titleWithoutExtension = track.title.replace(/\.mp3$/, '');
      const lyricsPath = `/lyrics/${this.currentAlbum.name}/${titleWithoutExtension}.txt`;
